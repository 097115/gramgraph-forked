    ...
    where <DB as plotters::prelude::DrawingBackend>::ErrorType: 'static
    {
        ...
        for cmd in &panel.commands {
            match cmd {
                ...
                DrawCommand::DrawBoxplot { x, width, min, q1, median, q3, max, outliers, style, legend, is_vertical } => {
                    let color = parse_color(&style.color, BLUE);
                    let outlier_color = parse_color(&style.outlier_color, color);
                    let alpha = style.alpha.unwrap_or(1.0);
                    let fill_style = color.mix(alpha).filled();
                    let stroke_style = color.mix(alpha).stroke_width(2);

                    let half_width = width / 2.0;

                    // Coordinates depend on orientation
                    // Vertical: X is category center, Y are stats
                    // Horizontal: Y is category center (passed as x), X are stats
                    let (min_pt, max_pt, box_p1, box_p2, med_p1, med_p2) = if *is_vertical {
                        (
                            (*x, *min), (*x, *max), // Whiskers
                            (*x - half_width, *q3), (*x + half_width, *q1), // Box Corners
                            (*x - half_width, *median), (*x + half_width, *median) // Median Line
                        )
                    } else {
                        (
                            (*min, *x), (*max, *x),
                            (*q1, *x - half_width), (*q3, *x + half_width),
                            (*median, *x - half_width), (*median, *x + half_width)
                        )
                    };

                    // 1. Whiskers (Lower and Upper)
                    // Draw separate lines to avoid drawing through the box (important for transparency)
                    let (box_start, box_end) = if *is_vertical { (box_p1.1.min(box_p2.1), box_p1.1.max(box_p2.1)) } else { (box_p1.0.min(box_p2.0), box_p1.0.max(box_p2.0)) };
                    
                    let lower_whisker_pts = if *is_vertical { vec![min_pt, (*x, box_start)] } else { vec![min_pt, (box_start, *x)] };
                    let upper_whisker_pts = if *is_vertical { vec![(*x, box_end), max_pt] } else { vec![(box_end, *x), max_pt] };

                    chart.draw_series(LineSeries::new(
                        lower_whisker_pts,
                        stroke_style.clone(),
                    )).context("Failed to draw lower whisker")?;

                    chart.draw_series(LineSeries::new(
                        upper_whisker_pts,
                        stroke_style.clone(),
                    )).context("Failed to draw upper whisker")?;

                    // Whisker Caps
                    let cap_width = width * 0.4; // Slightly narrower than the box
                    let cap_half = cap_width / 2.0;
                    
                    let (min_cap_p1, min_cap_p2) = if *is_vertical {
                        ((*x - cap_half, *min), (*x + cap_half, *min))
                    } else {
                        ((*min, *x - cap_half), (*min, *x + cap_half))
                    };

                    let (max_cap_p1, max_cap_p2) = if *is_vertical {
                        ((*x - cap_half, *max), (*x + cap_half, *max))
                    } else {
                        ((*max, *x - cap_half), (*max, *x + cap_half))
                    };

                    chart.draw_series(LineSeries::new(vec![min_cap_p1, min_cap_p2], stroke_style.clone()))
                        .context("Failed to draw min cap")?;
                    
                    chart.draw_series(LineSeries::new(vec![max_cap_p1, max_cap_p2], stroke_style.clone()))
                        .context("Failed to draw max cap")?;

                    // 2. Box
                    let box_anno = chart.draw_series(std::iter::once(Rectangle::new(
                        [box_p1, box_p2],
                        fill_style.clone(), 
                    ))).context("Failed to draw box")?;

                    if let Some(label) = legend {
                        box_anno.label(label)
                            .legend(move |(x, y)| Rectangle::new([(x, y - 5), (x + 15, y + 5)], fill_style.clone()));
                    }

                    // 3. Median Line (Contrast color, e.g., White or Black depending on fill?)
                    // Let's use WHITE for contrast on filled box.
                    chart.draw_series(LineSeries::new(
                        vec![med_p1, med_p2],
                        WHITE.mix(0.9).stroke_width(2),
                    )).context("Failed to draw median")?;

                    // 4. Outliers
                    if !outliers.is_empty() {
                         let out_size = style.outlier_size.unwrap_or(2.0) as i32;
                         let out_style = outlier_color.mix(alpha).filled();
                         
                         let points: Vec<(f64, f64)> = outliers.iter().map(|&val| {
                             if *is_vertical {
                                 (*x, val)
                             } else {
                                 (val, *x)
                         ...
                         chart.draw_series(points.iter().map(|p| {
                             Circle::new(*p, out_size, out_style.clone())
                         })).context("Failed to draw outliers")?;
                    }
                }
            }
        }
